...
import CoinbaseCryptoDisplay from '../../../components/Finance/CoinbaseDisplay';
...
export default function CryptoPage() {
  ...
  // Convert ticker format for SimpleCandlestickChart
  const convertTickerForCandlestick = (ticker: string): string => {
    // Remove X: prefix if present
    let formatted = ticker.startsWith('X:') ? ticker.substring(2) : ticker;

    // Replace USD with -USD format if needed
    if (formatted.endsWith('USD') && !formatted.includes('-')) {
      formatted = formatted.replace('USD', '-USD');
    }
    ...
    <div className="container py-10">
      ...
      <div className="container mx-auto">
        ...
        <div className="mb-6 grid grid-cols-1 lg:grid-cols-2 gap-4">
          ...
          {/* Real-time Candlestick Chart - Bottom left */}
          <div className="border rounded-lg overflow-hidden shadow-lg">
            <div className="p-4 bg-black border-b">
              <h2 className="text-lg font-bold">Real-time Candlestick Chart</h2>
            </div>
            <div className="h-96 w-full"> {/* Increase height and ensure full width */}
              <SimpleCandlestickChart
                ticker={convertTickerForCandlestick(selectedTicker)}
                key={`candlestick-${tickerChangeCounter}`}
